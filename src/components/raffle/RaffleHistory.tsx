...
const RaffleHistory = () => {
  ...
  return (
    <Window title="📜 PAST WINNERS 📜" className='h-fit'>
      <div className="p-4">
        {loading ? (
          ...
          <ul className="space-y-2">
            {history.map((item) => (
              <li key={item.id} className="bg-retro-gray-2 p-2 border-2 border-retro-gray-3">
                <p className="font-bold">Winner: <span className="font-pixel-operator-mono">{item.winner}</span></p>
                <div className="flex justify-between text-sm">
                  <span className="font-bold">Prize: <span className="font-pixel-operator-mono-bold text-retro-green">{item.prize}</span></span>
