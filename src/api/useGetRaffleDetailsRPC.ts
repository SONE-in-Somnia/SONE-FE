...
// Helper function to calculate raffle status
const calculateRaffleStatus = (
  ...
  if (winner !== '0x0000000000000000000000000000000000000000') {
    return RaffleStatus.COMPLETED;
  } else if (now < depositDeadline) {
    return RaffleStatus.IN_PROGRESS;
  } else {
    return RaffleStatus.DRAW_CLOSED;
  }
};
...
export const useGetRaffleDetailsRPC = (prizePoolAddress?: Address, userAddress?: Address) => {
  ...
  const fetchRaffleDetails = useCallback(async () => {
    ...
    try {
      ...
      if (targetAddress) {
        ...
        userWinRate = (userMulticallResults[3].status === 'success' ? Number(userMulticallResults[3].result) / 10000 : 0) as number; // Convert from parts-per-million to percentage
      }
      ...
      setData({
        ...
        isLoading: false,
        error: null,
        status: calculateRaffleStatus(Number(depositDeadline), Number(drawTime), winner), // Calculate and add status
      });
    } catch (err) {
  ...
  return { data, isLoading: data.isLoading, error: data.error, refetch: fetchRaffleDetails };
};