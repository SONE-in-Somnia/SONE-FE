...
/**
 * @title Kuro
 * @notice A simplified version of the Kuro game that supports MON and multiple LST tokens
 * @dev This contract supports both native MON and whitelisted ERC20 LST tokens
 */
contract Kuro is Ownable, ReentrancyGuard, Pausable {
    ...
    struct Deposit {
        uint256 roundId; // Round ID this deposit belongs to
        address depositor;
        address token; // address(0) for native MON, token address for ERC20
        uint256 amount; // amount in token's native decimals
        uint256 normalizedValue; // normalized value in MON equivalent
        uint256 entryCount;
        bool withdrawn;
    }
    ...
    struct SupportedToken {
        ...
        bool isActive;
        uint256 minDeposit;
        uint256 ratio; // Ratio to MON (in basis points, 10000 = 1:1, for depeg handling)
    }
    ...
    struct Round {
        ...
        mapping(address => uint256) tokenBalances; // Track balance per token in this round
    }

    /// @notice Structure for withdrawal data
    ...
    /// @notice Value of each entry in wei (MON equivalent)
    uint256 public valuePerEntry;

    /// @notice Round duration in seconds
    uint40 public roundDuration;

    /// @notice Protocol fee in basis points (1/100 of a percent)
    ...
    /// @notice Whether outflow of funds is allowed
    bool public outflowAllowed = true;

    /// @notice Minimum number of players required for a valid round
    uint256 public constant MINIMUM_PLAYERS_FOR_VALID_ROUND = 2;

    /// @notice Add a state variable for the keeper address
    ...
    /**
     * @notice Constructor to initialize the Kuro contract
     * @param _owner The owner of the contract
     * @param _roundDuration Duration of each round in seconds
     * @param _valuePerEntry Value of each entry in wei (MON equivalent)
     * @param _protocolFeeRecipient Protocol fee recipient address
     * @param _protocolFeeBp Protocol fee in basis points
     * @param _maximumNumberOfParticipantsPerRound Maximum participants per round
    ...
    constructor(
        ...
        // Initialize native MON token (address(0)) as supported by default
        supportedTokens[address(0)] = SupportedToken({
            isSupported: true,
            decimals: 18,
            isActive: true,
            minDeposit: _valuePerEntry,
            ratio: 10000 // 1:1 ratio
        });
    ...
    /**
     * @notice Universal deposit function for both MON and ERC20 tokens
     * @param token Token address (address(0) for native MON)
     * @param amount Amount to deposit (ignored for MON, use msg.value)
     */
    function deposit(
        address token,
        uint256 amount
        ...
        if (token == address(0)) {
            // Native MON deposit
            require(msg.value > 0, "MON deposit amount must be greater than 0");
            require(
                amount == 0,
                "Amount parameter should be 0 for MON deposits"
            );
            actualAmount = msg.value;
            normalizedValue = _normalizeTokenValue(token, actualAmount);
        } else {
            // ERC20 token deposit
            require(
                msg.value == 0,
                "Do not send MON when depositing ERC20 tokens"
            );
        ...
        _processDeposit(token, actualAmount, normalizedValue);
    }
     ...
     * @param normalizedValue Normalized value in MON equivalent
     */
    ...
    /**
     * @notice Normalize token value to MON equivalent with depeg ratio
     * @param token Token address
     * @param amount Token amount
     * @return Normalized value in MON equivalent
     */
    function _normalizeTokenValue(
        ...
        // Normalize to 18 decimals (MON equivalent)
     ...
     * @param ratio Ratio to MON in basis points (10000 = 1:1)
     */
    function addSupportedToken(
        ...
        require(token != address(0), "Cannot add native MON as ERC20 token");
        require(!supportedTokens[token].isSupported, "Token already supported");
        require(ratio > 0 && ratio <= 50000, "Invalid ratio"); // Max 5:1 ratio
     ...
     * @param decimals Token decimals
     * @param minDeposit Minimum deposit amount
     * @param ratio Ratio to MON in basis points
    ...
    function removeSupportedToken(address token) external onlyOwner {
        require(token != address(0), "Cannot remove native MON");
        require(supportedTokens[token].isSupported, "Token not supported");
    ...
    function getSupportedTokens() external view returns (address[] memory) {
        ...
        allTokens[0] = address(0); // Native MON
     ...
     * @param token Token address (address(0) for MON)
    ...
    /**
     * @notice Internal function to transfer tokens (MON or ERC20)
     */
